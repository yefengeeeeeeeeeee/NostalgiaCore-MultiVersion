...
class RailLogic {
	...
	public function canConnectTo(RailLogic $rail) {
		return $this->connectsTo($rail) || !(count($this->railPositions) == 2);
	}
	...
	public function place($b, $b1) {
		...
		if(!$this->isStraight) {
			if($hasZpos && $hasXpos && !($hasZneg && $hasXneg)) {
				$state = 6;
			}
			if($hasZpos && $hasXneg && !($hasZneg && $hasXpos)) {
				$state = 7;
			}
			if($hasZneg && $hasXneg && !($hasZpos && $hasXpos)) {
				$state = 8;
			}
			if($hasZneg && $hasXpos && !($hasZpos && $hasXneg)) {
				$state = 9;
			}
		}
		...
		if($state === 0) {
			if(RailBaseBlock::isRailBlock($this->level, $this->x, $this->y + 1, $this->z - 1)) {
				$state = 4;
			}
			if(RailBaseBlock::isRailBlock($this->level, $this->x, $this->y + 1, $this->z + 1)) {
				$state = 5;
			...
			if(RailBaseBlock::isRailBlock($this->level, $this->x + 1, $this->y + 1, $this->z)) {
				$state = 2;
			}
			if(RailBaseBlock::isRailBlock($this->level, $this->x - 1, $this->y + 1, $this->z)) {
				$state = 3;
