...
class Level {
	...
	public function isTopSolidBlocking($x, $y, $z) {
		...
		if(StaticBlock::getIsTransparent($id)) {
			return false;
		}
	...
	public function rayTraceBlocks(Vector3 $start, Vector3 $end) {
		...
		for($i = 0; $i <= 200; ++$i) {
			...
			if($v39) {
				$v21 = $v27 == 0 ? ($v15 - $start->x) * INF : ($v15 - $start->x) / $v27;
			}
			if($v40) {
				$v23 = $v29 == 0 ? ($v17 - $start->y) * INF : ($v17 - $start->y) / $v29;
			}
			if($v41) {
				$v25 = $v31 == 0 ? ($v19 - $start->z) * INF : ($v19 - $start->z) / $v31;
			}
		...
		for($x = $minX; $x < $maxX; ++$x) {
			for($y = $minY; $y < $maxY; ++$y) {
				for($z = $minZ; $z < $maxZ; ++$z) {
					...
					if($blockId == LAVA || $blockId == STILL_LAVA) {
						return true;
					}
				}
			}
	...
	public function updateNeighborsAt($x, $y, $z, $oldID) {
		...
		if($block) {
			StaticBlock::getBlock($block)::neighborChanged($this, $x - 1, $y, $z, $x, $y, $z, $oldID);
		}
		$block = $this->level->getBlockID($x + 1, $y, $z);
		if($block) {
			StaticBlock::getBlock($block)::neighborChanged($this, $x + 1, $y, $z, $x, $y, $z, $oldID);
		...
		if($block) {
			StaticBlock::getBlock($block)::neighborChanged($this, $x, $y - 1, $z, $x, $y, $z, $oldID);
		}
		$block = $this->level->getBlockID($x, $y + 1, $z);
		if($block) {
			StaticBlock::getBlock($block)::neighborChanged($this, $x, $y + 1, $z, $x, $y, $z, $oldID);
		...
		if($block) {
			StaticBlock::getBlock($block)::neighborChanged($this, $x, $y, $z - 1, $x, $y, $z, $oldID);
		}
		$block = $this->level->getBlockID($x, $y, $z + 1);
		if($block) {
			StaticBlock::getBlock($block)::neighborChanged($this, $x, $y, $z + 1, $x, $y, $z, $oldID);
	...
	public function onTick(PocketMinecraftServer $server, $currentTime) {
		...
		for($cX = 0; $cX < 16; ++$cX) {
			for($cZ = 0; $cZ < 16; ++$cZ) {
				...
				if(!isset($this->level->chunks[$index]) || $this->level->chunks[$index] === false) {
					continue;
				}
				for($c = 0; $c <= 20; ++$c) {
		...
		for($x = $minX; $x < $maxX; ++$x) {
			for($y = $minY; $y < $maxY; ++$y) {
				for($z = $minZ; $z < $maxZ; ++$z) {
					...
					if($blockAt == FIRE || $blockAt == STILL_LAVA || $blockAt == LAVA) {
						return true;
					}
				}
			}
