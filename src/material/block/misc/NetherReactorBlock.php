...
class NetherReactorBlock extends SolidBlock{
	...
	public function onActivate(Item $item, Player $player){
		...
		if($this->getMetadata() === 0 && $this->isCorrect($this->getX(),$this->getY(),$this->getZ()) && $this->getY() < 101 && NetherReactorBlock::$enableReactor){
			...
			$server->schedule(40, [$this, "glow"], 1);
			$server->schedule(60, [$this, "glow"], 2);
			$server->schedule(80, [$this, "glow"], 3);
			$server->schedule(140, [$this, "glow"], 4);
			$server->schedule(200, [$this, "spawnItems"], [0,15,2,true]); //200
			$server->schedule(260, [$this, "spawnItems"], [0,15,"checkPigmen",true]);
			$server->schedule(300, [$this, "spawnItems"], [0,15,"checkPigmen",true]);
			$server->schedule(340, [$this, "spawnItems"], [11,20,"checkPigmen",false]);
			$server->schedule(400, [$this, "spawnItems"], [0,10,"checkPigmen",false]);
			$server->schedule(500, [$this, "spawnItems"], [17,32, "checkPigmen",false]); //500
			$server->schedule(580, [$this, "spawnItems"], [17,32, "checkPigmen",false]);
			$server->schedule(620, [$this, "spawnItems"], [1,32, "checkPigmen",false]);
			$server->schedule(660, [$this, "spawnItems"], [1,32,"checkPigmen",false]);
			$server->schedule(700, [$this, "spawnItems"], [1,32,"checkPigmen",false]);
			$server->schedule(860, [$this, "glow"], 5);
			$server->schedule(880, [$this, "glow"], 6);
			$server->schedule(900, [$this, "glow"], 7);
			$server->schedule(920, [$this, "destroy"]);
			return true;
		}
		//}
