		...
		while(!$open->isEmpty())
		{
			...
			foreach(self::OFFSETS as $offset){
				...
				if(!$this->isLocBlocked($pmfLevel, $new)){
					if($newY > 0 && !$this->isLocBlocked($pmfLevel, $new - 1)){
						if(!$this->avoidFallDamage){
							...
							if($this->isLocBlocked($pmfLevel, $new - 2)){
								$new -= 1;
								$newY -= 1;
							}elseif($this->isLocBlocked($pmfLevel, $new - 3)){
								$new -= 2;
								$newY -= 2;
							}elseif($this->isLocBlocked($pmfLevel, $new - 4)){
								$new -= 3;
								$newY -= 3;
							}else{
								continue;
					...
					$points[] = $new;
				}elseif(!$this->isLocBlocked($pmfLevel, $current + 1) && !$this->isLocBlocked($pmfLevel, $new + 1)){
					$new += 1;
					$newY += 1;
